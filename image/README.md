...
2. `./image/prepare-sd.sh` with the SD still mounted, then boot the Pi
   on the network and run `sudo bash /boot/firmware/coresigner-provision.sh`.
...
The USB PSBT channel mounts itself: `99-coresigner-usb.rules` starts
`coresigner-usb@.service` when a stick is plugged in, which mounts its first
...
The result is the **dev image**: Core Signer at `/opt/coresigner`, verified Core
binary, ramdisk datadir at `/run/coresigner`, SPI enabled, `coresigner.service`
...
`PINS` is the whole release: OS image, Core version, and Core Signer commit,
...
- `provision.sh` sources `coresigner-PINS` from the FAT32 boot partition as
