...
C = [
 ("corky/hal.py", "DeviceDisplay.__init__", 2,
  "ST7789 panel init over SPI"),
 ("corky/hal.py", "DeviceDisplay.show", 2,
  "pushing a frame to the panel"),
 ("corky/hal.py", "DeviceButtons.pressed", 2,
  "the stuck-key break, needs GPIO"),
 ("corky/qrsource.py", "ImageQrSource.images", 3,
  "the abstract marker, raise NotImplementedError"),
 ("corky/qrsource.py", "CameraQrSource.images", 2,
  "picamera2 open, frame loop, teardown"),
 ("corky/qrsource.py", "DevQrSource.strings", 1,
  "dev paths with neither --qr-key nor --qr-psbt"),
 ("corky/qrsource.py", "DevQrSource.scan_psbt_frames", 1,
  "no PSBT file in a dev session"),
 ("corky/main.py", "_next_kind", 1,
  "called with a policy not in the ring"),
 ("corky/main.py", "_grid_move", 1,
  "given a key it does not handle"),
 ("corky/main.py", "Session.run", 1,
  "startup clear-out and power-off teardown raise"),
 ("corky/main.py", "Session.state_home", 1,
  "UP and LEFT on the grid; a flow raises a Core error"),
 ("corky/main.py", "Session.state_sign", 1,
  "Sign with no key loaded"),
 ("corky/main.py", "Session._tool_check_address", 1,
  "no key loaded; scan aborts, times out, or Core fails"),
 ("corky/main.py", "Session._check_address", 1,
  "no key loaded; a BIP21 URI; getaddressinfo raises"),
 ("corky/main.py", "Session._pick", 1,
  "UP in the shared list loop"),
 ("corky/main.py", "Session._export", 1,
  "leaving the export menu"),
 ("corky/main.py", "Session._export_one", 1,
  "the two refusals"),
 ("corky/main.py", "Session._export_qr", 1,
  "the QR card and its keys"),
 ("corky/main.py", "Session._export_text", 1,
  "the paged descriptor and its keys"),
 ("corky/main.py", "Session._page_addresses", 1,
  "the empty-order guard, a Core error, paging up"),
 ("corky/main.py", "Session._export_file", 1,
  "the write refusal"),
 ("corky/main.py", "Session._backup_paper", 1,
  "leaving the paper backup"),
 ("corky/main.py", "Session._discard", 1,
  "both ways out of the discard confirm"),
 ("corky/main.py", "Session._key_by_scan", 1,
  "the abort, and opening scanned descriptors"),
 ("corky/main.py", "Session._text_entry", 1,
  "the action bar: confirm, cancel, and C to jump"),
 ("corky/main.py", "Session._keymaterial", 1,
  "a rejected payload, and backing out"),
 ("corky/main.py", "Session._scan_until", 1,
  "the scan stream runs dry and restarts"),
 ("corky/main.py", "Session._guard_key_payload", 1,
  "a payload that is not text"),
 ("corky/main.py", "Session._key_xprv_typed", 1,
  "typing cancelled"),
 ("corky/main.py", "Session._tool_leak_check", 1,
  "the script fails to run; scrolling the report"),
 ("corky/main.py", "Session._show_backup", 1,
  "paging back off the first page"),
 ("corky/main.py", "Session._verify_backup", 1,
  "the refusal"),
 ("corky/main.py", "Session._check_page", 1,
  "leaving the check screen with and without errors"),
 ("corky/main.py", "Session._check_entry", 1,
  "the caret keys and the grid/entry focus swap"),
 ("corky/main.py", "Session.state_load", 1,
  "the no-channel result screen; leaving the chooser"),
 ("corky/main.py", "Session._load_by_stick", 1,
  "an unreadable file, and one still being written"),
 ("corky/main.py", "Session._load_by_qr.on_event", 1,
  "the large-frame and restart notices"),
 ("corky/main.py", "Session._load_by_qr", 1,
  "a stalled scan, and the restart after it"),
 ("corky/main.py", "Session._key_for", 1,
  "no key loaded, and no descriptor for the policy"),
 ("corky/main.py", "Session.state_review", 1,
  "the no-outputs result; moving and leaving review"),
 ("corky/main.py", "Session._sign_and_deliver", 1,
  "the result screen after a failed delivery"),
 ("corky/main.py", "main", 2,
  "the device branch that builds the real HAL"),
 ("corky/qrchannel.py", "frame_identity", 1,
  "a single-part UR has no sequence"),
 ("corky/qrchannel.py", "decode_image", 1,
  "a non-ascii QR is not a UR frame"),
 ("corky/qrchannel.py", "FrameAssembler.feed", 1,
  "an unexpected UR type, and a malformed container"),
 ("corky/qrchannel.py", "PsbtScan.feed", 1,
  "a frame accepted after the payload is complete"),
 ("corky/qrchannel.py", "PsbtScan._consider", 1,
  "the large-frame advisory repeats"),
 ("corky/qrchannel.py", "frames_to_images", 1,
  "a payload too large to render"),
 ("corky/screens.py", "_tracked", 1,
  "right-anchored tracked text"),
 ("corky/screens.py", "_fit", 1,
  "the character-by-character truncation"),
 ("corky/screens.py", "_fit_block", 1,
  "the smallest size still does not fit"),
 ("corky/screens.py", "scanning", 1,
  "the viewfinder: rotate, fill, crop and the progress bar"),
 ("corky/screens.py", "backup_page", 1,
  "the footer on the last page"),
 ("corky/signer.py", "_import", 1,
  "importdescriptors reports failures"),
 ("corky/signer.py", "open_session_descriptors", 1,
  "a multisig descriptor is refused"),
 ("corky/signer.py", "export_descriptor", 1,
  "a policy this key does not have"),
 ("corky/signer.py", "write_watch_only", 1,
  "the watch-only import fails"),
 ("corky/signer.py", "generate_wallet", 1,
  "generation fails and the wallet is dropped"),
 ("corky/signer.py", "master_xprv", 1,
  "the master key cannot be read"),
 ("corky/signer.py", "opens_wallet", 1,
  "no shape for the policy; a descriptor with no origin"),
 ("corky/signer.py", "master_fingerprint", 1,
